...
//				}*/
                ...
                if(StringUtils.isBlank(functionId)){
                    //连接后面没有functionId，需要去数据库中查询     update by hanxudong20170809
                    String requestUrl = ResourceUtil.getRequestPathWithParams(request);
                    CriteriaQuery cq = new CriteriaQuery(TSFunction.class);
                    try{
                        cq.eq("functionUrl",requestUrl);
                    }catch (Exception e){
                        e.printStackTrace();
                    }
                    cq.add();
                    List<TSFunction> retList = systemService.getListByCriteriaQuery(cq,false);
                    if(retList != null && !retList.isEmpty()){
                        functionId = retList.get(0).getId();
                    }
                }
				...
				if(!oConvertUtils.isEmpty(functionId)){
                    ...
                    if(operationCodes!=null){
                        Set<String> operationCodeStrs = new HashSet<String>();
                        for (String id : operationCodes) {
                            TSOperation tsOperation = systemService.getEntity(TSOperation.class, id);
                            if(tsOperation != null && tsOperation.getOperationType() == 0 && tsOperation.getStatus() == 0){
                                operationCodeStrs.add(tsOperation.getOperationcode());
                            }
                        }
                        request.setAttribute(Globals.OPERATIONCODESTRS, operationCodeStrs);
                    }
