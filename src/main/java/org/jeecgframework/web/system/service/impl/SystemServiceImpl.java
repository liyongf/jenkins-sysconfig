...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	...
	public void initAllTypeGroups() {
		...
		for (TSTypegroup tsTypegroup : typeGroups) {
			...
			List<TSType> types = this.commonDao.findHql("from TSType where TSTypegroup.id='"+tsTypegroup.getId()+"'order by typecode asc")/*.findByProperty(TSType.class, "TSTypegroup.id", tsTypegroup.getId())*/;
