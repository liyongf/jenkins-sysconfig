...
import org.jeecgframework.web.system.service.UpdateDataDic;
...
public class SystemController extends BaseController {
    @Autowired
    private UpdateDataDic updateDataDic;
	...
	public ModelAndView goTypeGrid(HttpServletRequest request) {
		...
		request.setAttribute("origin", ResourceUtil.getParameter("origin"));
...
//    }
    ...
    /** 刷新数据字典**/
    @RequestMapping(params = "reFleshAllTypeGroup")
    @ResponseBody
    public AjaxJson  reFleshAllTypeGroup(){
        String message = null;
        AjaxJson j = new AjaxJson();
        systemService.refleshTypeGroupCachDeep();
        message = "字典刷新成功";
        j.setMsg(message);
        return j;
    }
	...
	public AjaxJson saveTypeGroup(TSTypegroup typegroup, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		typegroup.setOrigin(Constants.TYPE_GROUP_ORIGIN_MINE);
...
//			depart.setOrgCode(orgCode);
     ...
     * Author：张赛超
     * */
    @RequestMapping(params = "updateDataDic")
    @ResponseBody
    public AjaxJson updateDataDic(){

        String url = ResourceUtil.getConfigByName("dataDictSyn");
        String json = null;
        ...
        msg = updateDataDic.save(json, msg);
    ...
    public ModelAndView addFiles(HttpServletRequest req) {
        ...
        String docSourceId = req.getParameter("docSourceId");
        if(StringUtils.isNotBlank(docSourceId)){
            req.setAttribute("docSourceId",docSourceId);
        }
        return new ModelAndView("system/document/filesUpload");
    }
    ...
    public AjaxJson saveFiles(HttpServletRequest request, HttpServletResponse response, TSDocument document) {
        ...
        TSType tsType = systemService.getType(typecode,"附件", tsTypegroup);
        ...
        if(StringUtils.isNotBlank(typeName)){
            document.setBusinessKey(typeName);
        }
        /*if(StringUtils.isNotBlank(baseType)){
            document.setInfoTypeId(baseType);
        }
        document.setBelongMine(belongMine);*/
        document.setStatus(1);
        document.setSubclassname(MyClassLoader.getPackPath(document));
        document.setCreatedate(DateUtils.gettimestamp());
        document.setTSType(tsType);

        document.setTSUser(ResourceUtil.getSessionUserName());
    ...
    public void documentQueryList(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		if (isSunRole){
			String sunSql = "select column_id from t_b_sunshine where table_name='t_s_document'";
    ...
    public AjaxJson delDocument(TSDocument document, HttpServletRequest request) {
        ...
        userService.saveOrUpdate(document);
     ...
     * 新闻法规文件列表
     */
    @RequestMapping(params = "documentListDoc")
