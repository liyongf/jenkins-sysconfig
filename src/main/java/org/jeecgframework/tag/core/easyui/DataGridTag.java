...
import org.jeecgframework.tag.vo.easyui.DataGridCombColumn;
...
public class DataGridTag extends TagSupport {
    ...
    protected List<DataGridCombColumn> combColumnList = new ArrayList<DataGridCombColumn>();// 列表操作显示 added by hansf at 2016-3-4
	...
	public void setColumn(String title, String field, Integer width,Integer showLen,String rowspan, 
        ...
        //列权限注释（edited by hanxudong at 2017-8-9 according to ykfxyk兖矿代码）
        ...
        Set<String> operationCodes = (Set<String>) super.pageContext.getRequest().getAttribute(Globals.OPERATIONCODESTRS);
        if (null!=operationCodes) {
            if(operationCodes.contains(field)){
                columnList.remove(dataGridColumn);
            }
        }
    ...
    /**
     *
     * <b>Summary: </b> setCombColumn(设置组合表头字段)
     *
     * @param title
     * @param field
     * @param width
     */
    public void setCombColumn(String title, String rowspan,
                              String colspan,String extendParams) {
        DataGridCombColumn dataGridColumn = new DataGridCombColumn();
        dataGridColumn.setColspan(colspan);
        dataGridColumn.setRowspan(rowspan);
        dataGridColumn.setTitle(title);
        dataGridColumn.setExtendParams(extendParams);
        combColumnList.add(dataGridColumn);
    }
...
//		}
	...
	public StringBuffer end() {
        ...
        sb.append("columns:[");
        this.getCombCol(sb); //added by hansf at 2016-3-4
        sb.append("[");
        this.getField(sb);
        sb.append("]],");
    ...
    /**
     * 拼接组合表头字段表头字段
     * added by hansf at 2016-3-4
     * @param sb
     */
    protected void getCombCol(StringBuffer sb) {
        if(combColumnList!=null&&combColumnList.size()>0){
            sb.append("[");
            int i = 0;
            for(DataGridCombColumn combcolumn :combColumnList){
                i++;
                sb.append("{colspan:\'" + combcolumn.getColspan() + "\',title:\'" + combcolumn.getTitle() + "\'");
                sb.append("}");
                // 去除末尾,
                if (i < combColumnList.size()) {
                    sb.append(",");
                }
            }
            sb.append("],");
